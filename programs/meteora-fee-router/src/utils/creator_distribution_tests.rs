...
use crate::{
    ...
    utils::creator_distribution::CreatorDistribution,
};
...
/// Comprehensive unit tests for creator distribution functionality
    ...
    fn test_validate_creator_payout_params_success() {
        let policy_config = create_test_policy_config();
        let distribution_progress = create_test_distribution_progress(true); // Day complete
    ...
    #[test]
    fn test_validate_creator_payout_params_failures() {
        let policy_config = create_test_policy_config();

        // Test 1: Day not complete - should now pass
        let distribution_progress = create_test_distribution_progress(false);
        let result = CreatorDistribution::validate_creator_payout_params(
            &policy_config,
            &distribution_progress,
            1000,
        ...
        // Test 2: Zero claimed amount - should now pass
        ...
        // Test 3: Invalid policy config
        let mut invalid_policy = policy_config;
        invalid_policy.investor_fee_share_bps = 10001; // Invalid BPS
        let result = CreatorDistribution::validate_creator_payout_params(
            &invalid_policy,
            &distribution_progress,
            1000,
...
/// Integration tests that combine creator distribution with other components
#[cfg(test)]
mod creator_distribution_integration_tests {
    use super::*;

    #[test]
    fn test_creator_distribution_with_investor_distribution() {
        let policy_config = create_test_policy_config();
        let claimed_quote = 10000u64;
